...
let generate () =
  ...
  for i = 1 to n do
    ...
    s1 := S.add k !s1;
    is1 := IS.add (C.cons k) !is1;
    let k = Random.int n in
    s2 := S.add k !s2;
    is2 := IS.add (C.cons k) !is2;
  done;
  c1 := IS.cardinal !is1;
  c2 := IS.cardinal !is2;
  update_time ();
...
let test_mem () =
  for i = 1 to n do
    let k = Random.int n in
    assert_equal ~msg:"test 1" (S.mem k !s1) (IS.mem (C.cons k) !is1);
    assert_equal ~msg:"test 2" (S.mem k !s2) (IS.mem (C.cons k) !is2);
  done;
  update_time ();
;;
let test_mem_iter () =
  ...
  S.iter (fun k ->
          assert_bool "test 2" (IS.mem (C.cons k) !is1)
    ) !s1;
  IS.iter (fun k_c ->
          assert_bool "test 3" (S.mem k_c.data !s2)
    ) !is2;
  S.iter (fun k ->
          assert_bool "test 4" (IS.mem (C.cons k) !is2)
    ) !s2;
  update_time ();
;;
...
let test_remove () =
  ...
  assert_equal ~msg:"test 1" (S.cardinal !s) (IS.cardinal !is);  
  IS.iter (fun k_c ->
    assert_bool "test 2" (S.mem k_c.data !s);
    ) !is;
  S.iter (fun k ->
    is := IS.remove (C.cons k) !is;
    ) !s;
  assert_equal ~msg:"test 3" (IS.cardinal !is) 0;
  update_time ();
...
let test_equal () =
  ...
  S.iter (fun k -> is := IS.add (C.cons k) !is) !s1;
  update_time ();
...
let _ =
  ...
  if not (was_successful (run_test_tt ~verbose:!verbose corr_suite)) then
    exit 1;
  print_memory_usage ();
  print_cpu_time ();
;;